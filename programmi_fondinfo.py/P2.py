...
class _3inarow(BoardGame):
    ...
    def unsolvable(self) -> bool:
        copia = self._matrix
        m, rig, cln, solve = copia, self._righe, self._colonne, False
        ...
        if "-" in m:
            ...
            for a in range(0,rig): 
                for b in range(0,cln):
                    if (m[a * cln + b] == "w"): #righe
                        m[a * cln + b] = "W"
                    elif (m[a * cln + b] == "b"):
                        m[a * cln + b] = "B"
                    if (m[b * cln + a] == "w"): #colonne
                        m[b * cln + a] = "W"
                    elif (m[b * cln + a] == "b"):
                        m[b * cln + a] = "B"
                    if (m[a * cln + b] == "W"): #righe
        ...
        if (row != rig) or (col != cln) or (r_row >= rig) or (r_col >= cln):
            solve = True
        return solve
